...
import gr.aueb.cf.ch18.mobilecontacts.dto.*;
...
public class Main {
    ...
    // Wiring (συνδέουμε το dao και το service layer)
    ...
    public static void main(String[] args) {
        ...
        while(true) {
            ...
            if (choice.matches("[qQ]")) {   // if it is equal to q or Q
    ...
    public static void processWithChoice(String choice) {
        ...
        switch (choice) {
            case "1":
                 ...
                 break;
            case "2":
                System.out.println("Παρακαλώ εισάγετε το id της επαφής προς ενημέρωση:");
                id = scanner.nextLong();
    ...
    private static void printContacts(List<MobileContactReadOnlyDTO> contacts) {
        contacts.forEach(mb -> System.out.println(mb)); // or (System.out::println);
    }
    ...
    private static MobileContactUpdateDTO mapToContactUpdateDto(MobileContact mobileContact) {
        return new MobileContactUpdateDTO(mobileContact.getId(), mapToUserDetailsUpdateDto(mobileContact.getUserDetails()), mobileContact.getPhoneNumber());
    }
    // here
    private static UserDetailsUpdateDTO mapToUserDetailsUpdateDto(UserDetails userDetails) {
        return new UserDetailsUpdateDTO(userDetails.getId(), userDetails.getFirstname(), userDetails.getFirstname());
    }

    private static UserDetailsInsertDTO mapToUserDetailsInsertDto(UserDetails userDetails) {
        return new UserDetailsInsertDTO(userDetails.getId(), userDetails.getFirstname(), userDetails.getFirstname());
    }

    private static MobileContactInsertDTO mapToContactInsertDto(MobileContact mobileContact) {
        return new MobileContactInsertDTO(mobileContact.getId(), mapToUserDetailsInsertDto(mobileContact.getUserDetails()), mobileContact.getPhoneNumber());
    }

    // here
    private static MobileContactReadOnlyDTO mapToContactReadOnlyDto(MobileContact mobileContact) {
        return new MobileContactReadOnlyDTO(mobileContact.getId(), mapToUserDetailsReadOnlyDto(mobileContact.getUserDetails()), mobileContact.getPhoneNumber());
    }
    // here
    private static UserDetailsReadOnlyDTO mapToUserDetailsReadOnlyDto(UserDetails userDetails) {
        return new UserDetailsReadOnlyDTO(userDetails.getId(), userDetails.getFirstname(), userDetails.getFirstname());
    }
